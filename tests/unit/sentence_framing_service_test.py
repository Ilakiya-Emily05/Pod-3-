...
async def test_submit_response_success():
    ...
    mock_next_result.scalar_one_or_none.side_effect = [next_id, None] # One for next, one for fallback if needed
    
    db.execute.side_effect = [mock_ex_result, mock_next_result]
    
    payload = SentenceSubmissionCreate(
        exercise_id=exercise_id,
        response="Testing feedback",
        time_taken_secs=100
    )
    ...
    with patch("app.services.sentence_framing_service.evaluate_sentence_response", new_callable=AsyncMock) as mock_ai:
        ...
        assert submission._next_exercise_id == next_id
