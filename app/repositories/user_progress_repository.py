...
class UserProgressRepository:
    ...
    async def get_by_user_topic(self, user_id: str, module_name: str, topic: str, subtopic: str) -> Optional[UserProgress]:
    ...
    async def create(self, user_id: str, module: str, topic: str, subtopic: str, total_attempts: int = 0, correct_attempts: int = 0, time_spent_secs: int = 0) -> UserProgress:
        up = UserProgress(
            user_id=user_id,
            module_name=module,
            topic=topic,
    ...
    # Streaks
    async def get_streaks_for_user(self, user_id: str) -> Optional[UserStreaks]:
    ...
    async def create_streak(self, user_id: str, current_streak: int, longest_streak: int, last_activity_date):
        s = UserStreaks(user_id=user_id, current_streak=current_streak, longest_streak=longest_streak, last_activity_date=last_activity_date)
        self.db.add(s)
        await self.db.commit()
