...
class TestSessionRepository:
    ...
    def create_session(self, user_id: str, topic: str, subtopic: str) -> TestSession:
        session = TestSession(
            user_id=user_id,
            current_topic=topic,
    ...
    def get_by_id(self, session_id: UUID) -> TestSession | None:
        return self.db.query(TestSession).filter(TestSession.id == session_id).first()

    def get_active_session(self, user_id: str) -> TestSession | None:
