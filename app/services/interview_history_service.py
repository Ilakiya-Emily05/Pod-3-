"""
Interview History Service — Sprint 2 Task 1

Handles:
- Session history with pagination, filtering, sorting
- Session replay with full question breakdown
- Progress tracking with score progression, milestones, consistency
"""
import logging
...
async def _build_session_summary(db: AsyncSession, s: InterviewSession) -> dict:
    """Build the per-session dict for the history list."""
    ...
    has_recordings = rec_result.scalar_one() > 0
    ...
    return {
        ...
        "performance_level": performance_level(s.overall_score),
...
async def _build_summary(db: AsyncSession, user_id: str, session_list: list[dict]) -> dict:
    ...
    return {
        "total_interviews": sum(1 for _ in session_list),
...
async def get_improvement_history(db: AsyncSession, user_id: str) -> dict:
    ...
    score_progression = [
        {"date": s.created_at.strftime("%Y-%m-%d"), "score": s.overall_score, "type": s.interview_type}
        for s in sessions
...
async def _build_milestones(db: AsyncSession, user_id: str) -> list[dict]:
    ...
    return [
        {"name": m.milestone_name, "achieved_at": m.achieved_at.strftime("%Y-%m-%d")}
