...
async def _build_session_summary(db: AsyncSession, s: InterviewSession) -> dict:
    q_count = (
        await db.execute(select(func.count(UserResponse.id)).where(UserResponse.session_id == s.id))
    ).scalar_one()
    has_recordings = (
        await db.execute(
...
async def _build_question_detail(db: AsyncSession, index: int, row: object) -> dict:
    ...
    skill_keyword = (
        await db.execute(select(KeySkill.keyword).where(KeySkill.id == q.skill_id))
    ).scalar_one_or_none()
