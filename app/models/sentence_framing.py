...
class SentenceExercise(Base, TimestampMixin):
    ...
    exercise_type: Mapped[str] = mapped_column(String(50), nullable=False)  # 'fill_in_blank', 'reorder', 'free_form'
    difficulty: Mapped[str] = mapped_column(String(20), nullable=False)
    industry: Mapped[str | None] = mapped_column(String(50), nullable=True)
    scenario: Mapped[str] = mapped_column(Text, nullable=False)
    context: Mapped[dict] = mapped_column(JSONB, nullable=False)  # stores sender_role, recipient, tone
    template: Mapped[str | None] = mapped_column(Text, nullable=True) # for fill-in-blank or reorder
