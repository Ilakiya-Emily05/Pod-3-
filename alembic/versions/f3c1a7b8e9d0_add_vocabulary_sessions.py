...
down_revision: str | Sequence[str] | None = "83b909912715"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"], ["users.id"], name=op.f("vocabulary_sessions_user_id_fkey")
        ),
        sa.PrimaryKeyConstraint("session_id", name=op.f("vocabulary_sessions_pkey")),
    )
    op.create_index(
        op.f("ix_vocabulary_sessions_user_id"), "vocabulary_sessions", ["user_id"], unique=False
    )
